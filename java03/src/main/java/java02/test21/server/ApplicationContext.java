...
/*
 ...
 =>로그레벨
	...
	=>로그 출력 레벨 설정
	1. TRACE로 설정하면 TRACE 이하 모든 로그들을 설정한다.
	2. 만약 WARN레벨로 설정하면 WARN 이하, ERROR, FATAL 레벨까지도 출력한다.
...
public class ApplicationContext {
	...
	private void prepareObjects(String packageName) throws Exception {
		...
		for (Class<?> clazz : clazzList) {
			...
			// 클래스 관리자에게서 Component 애노테이션 정보얻기
		...
		for (Object obj : objPool.values()) {
			...
			for (Method m : methods) {
				...
				if (m.getName().startsWith("set")) {
					...
					//parameter = m.getParameters()[0]; //JDK1.8이상

					//System.out.println("                   " + parameter.getType().getName());
					// setter 메서드의 파라미터 타입과 일치하는 객체를 찾는다.
					//dependency = findDependency(parameter.getType()); //JDK1.8이상
