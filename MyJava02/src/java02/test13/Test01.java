...
//       =>public , default, protected. private
 ...
 class Test01 {
  ...
  static void x() {
    i = 100; // ok
    // j = 100; //error = >static 메서드나 블록은 this라는 내장변수가 없다.
  }
  ...
  static class TopLevelInnerClass {
    ...
    // static 영역에서는 오로지 바깥 클래스의 static 멤버(변수, 메서드)
    // 만 접근할 수 있다.

     public void test() {
      i = 10; // ok => 바깥 클래스의 static 변수를 사용할 수 있다.
      // j = 200; //error => 바같 클래스의 인스턴스 주소를 알 수 없다.
    }
  }
  ...
  class MemberInnerClass {
    ...
    public void test() {
      i = 100; // ok 당연히 non-static 블록에서는 static 멤버 사용 가능

      // ok member 이너 클래스에서는 바깥 클래스의 인스턴스 멤버를
      // 사용할 수 있다.
      j = 200;
      //위 코드는 컴파일 할 때 다음과 같이 바뀐다.
      //Test01.this.j = 400; 
      //만약 이너 클래스에 같은 이름을 가진 변수가 있다면,
      //구분하기 위해 개발자가 다음과 같이 명시적으로  선언해야한다. 
      Test01.this.j = 400; 
    }
  }
