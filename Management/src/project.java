...
class StudentManager {
 ...
 public void viewInfo(String index) {
  ...
  System.out.println("이름: "
    + infoStorage.get(Integer.parseInt(index)).name);
  System.out.println("국어: "
    + infoStorage.get(Integer.parseInt(index)).kor);
  System.out.println("영어: "
    + infoStorage.get(Integer.parseInt(index)).eng);
  System.out.println("수학: "
    + infoStorage.get(Integer.parseInt(index)).math);
  System.out.println("총점: "
    + infoStorage.get(Integer.parseInt(index)).sum);
 ...
 public void Delete(String index) {
  if (Integer.parseInt(index) >=0 && Integer.parseInt(index) < infoStorage.size() ) {
...
public class project {
 public static void main(String[] args) {
  ...
  while (true) {
   ...
   switch (arr[0]) {
   case "help": MenuViewer.showMenu(); break;
   case "list":  manager.listInfo(); break;
   case "view":  manager.viewInfo(arr[1]); break;
   case "add":   manager.inputData(arr[1], arr[2], arr[3], arr[4]); break;
   case "delete": manager.Delete(arr[1]); break;
   case "update":  manager.update(arr[1]); break;
   case "exit": manager.storeToFile(); return;
   default : System.out.println("잘못된 명령어입니다."); break;
   }
  }
 }
}
